...
class DiGraph {
 ...
 public:
  ...
  DiGraph(
      const std::function<std::size_t(const NodePtr &)> &node_hash,
  ...
  void RemoveEdge(const std::string &s_name, const std::string &t_name,
                  const double w = 1.0) {
    RemoveEdge(std::make_shared<Node>(s_name), std::make_shared<Node>(t_name),
               w);
  }
